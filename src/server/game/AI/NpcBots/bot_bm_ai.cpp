        ...
        void CriticalStrikeFinish(uint64 targetGuid, CalcDamageInfo* dinfo, bool /*windwalk*/)
        {
            ...
            for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
            {
                me->SendSpellNonMeleeDamageLog(target, CRITICAL_STRIKE_1,
                    dinfo->Damages[i].Damage + dinfo->Damages[i].Absorb + dinfo->Damages[i].Resist + dinfo->Blocked,
                    SPELL_SCHOOL_MASK_NORMAL, dinfo->Damages[i].Absorb, dinfo->Damages[i].Resist, true, dinfo->Blocked, true);
                CleanDamage cl(0, 0, BASE_ATTACK, MELEE_HIT_CRIT);
                Unit::DealDamage(me, target, dinfo->Damages[i].Damage, &cl);
            }
